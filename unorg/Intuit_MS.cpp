...
void solve()
{
    ...
    for(ll i=0;i<n;i++){
        for(ll j=0;j<m;j++){
            if(i > 0){
                dp[i][j] = max(dp[i][j], arr[i][j] - arr[i-1][j] + dp[i-1][j]);
                dp[i][j] = max(dp[i][j], arr[i][j] - arr[i-1][j]);
            }
            if(j > 0){
                dp[i][j] = max(dp[i][j], arr[i][j] - arr[i][j-1] + dp[i][j-1]);
                dp[i][j] = max(dp[i][j], arr[i][j] - arr[i][j-1]);
            }
        }
    }
    for(ll i=0;i<n;i++){
