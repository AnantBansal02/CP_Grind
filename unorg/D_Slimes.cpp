#pragma GCC optimize("-Ofast")
#pragma GCC target("sse,sse2,sse3,ssse3,sse4,popcnt,abm,mmx,avx,avx2,fma")
#pragma GCC optimize("unroll-loops")
#include <bits/stdc++.h>
using namespace std;
typedef long long ll;
#define dbg(x) cout << #x << " = " << x << "\n"
#define pb push_back
#define fi first
#define se second
#define INF 1e18
#define all(x) (x).begin(), (x).end()
#define set_bits(x) __builtin_popcountll(x)

ll power(ll x, ll y, ll p){
    ll res = 1;x = x % p;while (y > 0){if (y & 1)res = (res * x) % p;y = y >> 1;x = (x * x) % p;}return res;
}
ll modInverse(ll n, ll p){
    return power(n, p - 2, p);
}
vector<ll> fac;
void factorial(int n, int MOD){
    fac.resize(n + 1);fac[0] = 1;for (int i = 1; i < n; i++){fac[i] = fac[i - 1] * i % MOD;}
}
ll ncrmod(ll n, ll r, ll p){
    if (n < r)return 0;
    if (r == 0)return 1;
    return (((fac[n] * modInverse(fac[r], p)) % p) * modInverse(fac[n - r], p)) % p;
}
vector<ll> primes;
void seive(ll n){
    bool prime[n + 1];memset(prime, true, sizeof(prime));
    for (int p = 2; p * p <= n; p++) {
        if (prime[p] == true) {for (int i = p * p; i <= n; i += p)prime[i] = false;}
    }
    for (int p = 2; p <= n; p++)if(prime[p])primes.push_back(p);
}
ll getNoOfFactors(ll num) {
    if( num < 2 ) { return 0;}ll cnt = 0;
    for ( auto x : primes ) {while( num % x == 0 ) {cnt++;num /= x ;}}
    if( num > 1){cnt++;}
    return cnt;
}
void solve()
{
    ll n;cin>>n;
    vector<ll> arr(n);
    for(auto &it : arr){
        cin>>it;
    }
    vector<ll> ans(n, -1);
    vector<ll> check(n+1, 0);
    for(ll i=1;i<n;i++){
        if(arr[i] != arr[i-1]){
            check[i+1] = 1;
        }
        check[i+1] += check[i];
    }
    // for(auto it : check){
    //     cout<<it<<" ";
    // }
    // cout<<"\n";
    vector<ll> pre(n+1,0);
    vector<ll> suff(n+1,0);
    for(ll i=1;i<=n;i++){
        pre[i] = arr[i-1] + pre[i-1];
    }
    suff[n] = arr[n-1];
    for(ll i=n-1;i>=1;i--){
        suff[i] = arr[i-1] + suff[i+1];
    }
    for(ll i=1;i<=n;i++){
        ll low = i+1;
        ll high = n;
        ll curr = INT_MAX;
        if((i < n and arr[i-1] < arr[i]) || (i-2 >= 0 and arr[i-1] < arr[i-2])){
            ans[i-1] = 1;
            continue;
        }
        while(low <= high){
            ll mid = (low + high) >> 1;
            if(pre[mid] - pre[i] > arr[i-1] and (check[mid] - check[i+1] > 0 || mid - i == 1)){
                curr = mid - i;
                high = mid - 1;
            }
            else{
                low = mid + 1;
            }
        }
        low = 1;
        high = i-1;
        ll curr2 = INT_MAX;
        while(low <= high){
            ll mid = (low + high) >> 1;
            if(suff[mid] - suff[i] > arr[i-1] and (abs(check[mid] - check[i-1]) > 0 || i - mid == 1)){
                curr2 = i - mid;
                low = mid + 1;
            }
            else{
                high = mid - 1;
            }
        }
        ans[i-1] = min(curr, curr2);
    }
    for(ll i=0;i<n;i++){
        if(ans[i] == INT_MAX){
            ans[i] = -1;
        }
        cout<<ans[i]<<" ";
    }
    cout<<"\n";
}
int main()
{
    ios_base::sync_with_stdio(false);
    cin.tie(NULL);
    // factorial(1e5,1e9+7);
    // seive(1e5);
    ll t = 1;
    cin >> t;
    while(t--){
        solve();
    }
    return 0;
}