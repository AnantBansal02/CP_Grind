...
void solve()
{
    ...
    vector<unordered_map<string, ll>> mp(n+1);
    function<ll(ll, string)> f = [&](ll idx, string temp) ->ll {
        if(temp == t){
            return 0;
        }
        if(idx == n){
        ...
        if(mp[idx].count(temp)){
            return mp[idx][temp];
        }
        ll ans = INT_MAX;
        for(auto it : arr[idx]){
            if(check(temp + it)){
                ans = min(ans, 1 + f(idx+1, temp + it));
            }
        }
        ans = min(ans, f(idx+1, temp));
