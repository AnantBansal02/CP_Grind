...
void solve()
{
    ...
    double ans = INT_MAX;
    ans = min(ans, max(dist(ax,ay,px,py), dist(ax,ay, 0, 0)));
    ans = min(ans, max(dist(bx,by,px,py), dist(bx,by, 0, 0)));
    double val = max(dist(ax,ay,0,0), dist(bx,by,px,py));
    if(dist(ax,ay,bx,by)/2 >= val){
        ans = min(ans, dist(ax,ay,bx,by)/2);
    }
    else{
        ans = min(ans, val);
    }
    val = max(dist(ax,ay,px,py), dist(bx,by,0,0));
    if(dist(ax,ay,bx,by)/2 >= val){
        ans = min(ans, dist(ax,ay,bx,by)/2);
    }
    else{
